...
const ObtenerPermisosPorRolYRecurso = async (CodigoRol, NombreRecurso, NombrePermiso) => {
  try {
    ...
    if (!recursoData || !permisoData) {
      return LanzarError('El permiso no existe.', 404, 'Alerta');
    }
    ...
    if (relacionData.Estatus !== 1) {
      return LanzarError('Permiso inactivo.', 403, 'Alerta');
    }

    // Validar que el rol esté activo
    const rolData = await PermisoRolRecursoModelo.findOne({ where: { CodigoRol } });
    if (!rolData) {
      return LanzarError(`El rol "${CodigoRol}" no existe.`, 404, 'Alerta');
    }
    if (rolData.Estatus !== 1) {
      return LanzarError(`El rol "${CodigoRol}" está inactivo.`, 403, 'Alerta');
    }

    // Validar que el recurso esté activo
    if (recursoData.Estatus !== 1) {
      return LanzarError(`El recurso "${NombreRecurso}" está inactivo.`, 403, 'Alerta');
    }

    // Validar que el permiso esté activo
    if (permisoData.Estatus !== 1) {
      return LanzarError(`El permiso "${NombrePermiso}" está inactivo.`, 403, 'Alerta');
    }
    ...
    if (!datos || datos.length === 0) {
      return LanzarError(`No se encontraron permisos para el rol ${CodigoRol} y recurso ${NombreRecurso}`, 404, 'Alerta');
    }
