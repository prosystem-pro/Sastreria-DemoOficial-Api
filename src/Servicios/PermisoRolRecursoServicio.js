...
const Crear = async (Datos) => {
  try {
    ...
    if (!CodigoRol) return LanzarError('Falta CódigoRol en los datos', 400);
    if (!Array.isArray(items) || items.length === 0) return LanzarError('No hay items para crear', 400);
    ...
    if (registrosFiltrados.length === 0) return LanzarError('Todos los permisos ya existen para este rol', 409);
...
const EliminarPorRol = async (CodigoRol) => {
  try {
    if (!CodigoRol || isNaN(CodigoRol)) return LanzarError('Código de rol inválido', 400);

    const registros = await Modelo.findAll({ where: { CodigoRol } });
    if (registros.length === 0) return LanzarError(`No se encontraron registros para el rol ${CodigoRol}`, 404);
...
const EliminarPorRolRecurso = async (CodigoRol, CodigoRecurso) => {
  try {
    if (!CodigoRol || isNaN(CodigoRol)) return LanzarError('Código de rol inválido', 400);
    if (!CodigoRecurso || isNaN(CodigoRecurso)) return LanzarError('Código de recurso inválido', 400);
...
const EliminarPorPermisoRolRecurso = async (CodigoRol, CodigoRecurso, CodigoPermiso) => {
  try {
    if (!CodigoRol || isNaN(CodigoRol)) return LanzarError('Código de rol inválido', 400);
    if (!CodigoRecurso || isNaN(CodigoRecurso)) return LanzarError('Código de recurso inválido', 400);
    if (!CodigoPermiso || isNaN(CodigoPermiso)) return LanzarError('Código de permiso inválido', 400);
...
const ObtenerPermisosFrontEnd = async (CodigoRol) => {
  try {
    ...
    if (registros.length === 0) {
      return LanzarError(`No se encontraron permisos para el rol ${CodigoRol}`, 404);
    }
